...
router.get('/:id/leaderboard', requireAuth, async (req: AuthRequest, res, next) => {
  try {
    ...
    const allSubmissions = await SubmissionModel.find({ contestId: contest._id })
      .populate('studentId', 'name email');

    // Deduplicate: Keep best submission per student
    const bestSubmissionsMap = new Map<string, any>();
    ...
    for (const sub of allSubmissions) {
      ...
      // Comparison logic: Score > Accuracy > CompletedAt
      let isBetter = false;
      if (sub.score > existing.score) {
        isBetter = true;
      } else if (sub.score === existing.score) {
        if (sub.accuracy > existing.accuracy) {
          isBetter = true;
        } else if (sub.accuracy === existing.accuracy) {
          if (new Date(sub.completedAt) < new Date(existing.completedAt)) {
            isBetter = true;
          }
        }
      }
    ...
    const uniqueSubmissions = Array.from(bestSubmissionsMap.values());

    // Sort for leaderboard: Score desc, then Accuracy desc, then Time asc
    const sortedSubmissions = uniqueSubmissions.sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      if (b.accuracy !== a.accuracy) return (b.accuracy || 0) - (a.accuracy || 0);
      return new Date(a.completedAt).getTime() - new Date(b.completedAt).getTime();
    });

    const leaderboard = sortedSubmissions.slice(0, 100).map((sub, index) => ({
      rank: index + 1,
...
router.get('/:id/stats', requireAuth, requireRole(['admin']), async (req: AuthRequest, res, next) => {
  try {
    ...
    // Fetch all submissions to handle deduplication in memory
    // (Assuming contest size isn't massive; if so, aggregation pipeline would be better)
    const allSubmissions = await SubmissionModel.find({ contestId: contest._id })
      .populate('studentId', 'name email');
    ...
    for (const sub of allSubmissions) {
      // Skip if studentId is null (deleted user)
      if (!sub.studentId) continue;
      ...
      // Comparison logic: Higher Score > Higher Accuracy > Earlier Completion
      let isBetter = false;
      if (sub.score > existing.score) {
        isBetter = true;
      } else if (sub.score === existing.score) {
    ...
    const uniqueSubmissions = Array.from(bestSubmissionsMap.values());
    const totalParticipants = uniqueSubmissions.length;
    ...
    const scores = uniqueSubmissions.map(s => s.score);
    ...
    scores.forEach(score => {
      // Calculate percentage based on contest.numQuestions
      // Fallback to 0 if numQuestions is missing or 0 to avoid Infinity
      const maxScore = contest.numQuestions || 1;
    ...
    const participantIds = new Set(uniqueSubmissions.map(s => (s.studentId as any)._id.toString()));
