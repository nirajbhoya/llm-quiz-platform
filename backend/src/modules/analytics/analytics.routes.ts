...
router.get('/student/me/contests', requireAuth, requireRole(['student']), async (req: AuthRequest, res, next) => {
  try {
    ...
    // Only count as missed if the contest is COMPLETED and was not attempted
    const missedContests = contestEntries.filter((c) => !c.attempted && c.status === 'COMPLETED').length;
...
router.get('/parent/child/:childId/contests', requireAuth, requireRole(['parent']), async (req: AuthRequest, res, next) => {
  try {
    ...
    // 1. Get all contests relevant to the student (Only LIVE and COMPLETED)
