...
import { validateRealEmail } from '../../services/email-validation.service';
...
export const registerStudent = async (req: Request, res: Response) => {
  ...
  const { name, email, password, boardCode, standard, gender, parentEmail, parentName } = req.body as {
    name: string;
    email: string;
    password: string;
    boardCode?: string;
  ...
  console.log('[registerStudent] Initializing registration for:', email);

  // 1. Real email validation for Student
  const studentEmailValidation = await validateRealEmail(email);
  if (!studentEmailValidation.isValid) {
    console.log('[registerStudent] Student email rejected:', studentEmailValidation.error);
    return res.status(400).json({ error: studentEmailValidation.error });
  }

  // 2. Real email validation for Parent
  const parentEmailValidation = await validateRealEmail(parentEmail);
  if (!parentEmailValidation.isValid) {
    console.log('[registerStudent] Parent email rejected:', parentEmailValidation.error);
    return res.status(400).json({ error: `Parent email: ${parentEmailValidation.error}` });
  }

  console.log('[registerStudent] parsed body:', { name, email, boardCode, standard, gender, parentEmail, parentName });
