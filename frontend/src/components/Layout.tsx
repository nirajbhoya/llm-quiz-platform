...
export default function Layout({ children }: LayoutProps) {
  ...
  const getNavLinks = () => {
    ...
    if (user.role === 'student') {
      return [
        ...
        { to: '/student/community-doubts', label: 'Community' },
        { to: '/student/learning', label: 'Practice' },
        { to: '/student/contests', label: 'Contests' },
        { to: '/student/doubt-session', label: 'AI Q&A' },
      ...
      return [
        ...
        { to: '/admin/doubt-management', label: 'Doubts' },
      ];
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-purple-500 selection:text-white relative">
      {/* Starry Background Effect - Persistent */}
      ...
      {/* Subtle global glow */}
      <div className="fixed top-0 left-0 w-full h-[500px] bg-purple-900/10 blur-[100px] pointer-events-none"></div>

      <nav className="sticky top-0 z-50 bg-gray-900/50 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex">
              <div className="flex-shrink-0 flex items-center">
                <Link to="/" className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-indigo-400">
                  QuizGenius
                </Link>
              </div>
              <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
                {getNavLinks().map((link) => (
                  <Link
                    key={link.to}
                    to={link.to}
                    className="border-transparent text-gray-300 hover:text-white hover:border-purple-500 inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium transition-colors"
                  >
                    {link.label}
                  </Link>
                ))}
            ...
            <div className="flex items-center">
              {user && (
                <div className="relative">
                  ...
                  {isProfileOpen && (
                    <div
                      className="origin-top-right absolute right-0 mt-2 w-64 rounded-xl shadow-[0_0_30px_rgba(0,0,0,0.5)] bg-gray-900 border border-white/10 ring-1 ring-black ring-opacity-5 focus:outline-none z-50 backdrop-blur-xl"
                      role="menu"
                              ...
                              <div className="px-4 py-2 space-y-2 text-sm">
                                <div>
                                  ...
                                  <select
                                    ...
                                    className="w-full px-2 py-1 border border-gray-700 rounded-md bg-black/50 text-white focus:outline-none focus:ring-1 focus:ring-purple-500"
                                  >
                                ...
                                <div className="flex justify-end space-x-2 pt-1">
                                  <button
                                    ...
                                    className="px-2 py-1 text-xs border border-gray-700 rounded-md text-gray-400 hover:bg-white/5"
                                  >
                                    Cancel
                                  </button>
                                  <button
                                    ...
                                    className="px-3 py-1 text-xs bg-purple-600 text-white rounded-md disabled:opacity-50 hover:bg-purple-700"
                                  >
