...
import type { Quiz, Question } from './learning.service';
...
export const contestsService = {
  ...
  generateCandidateQuestions: async (id: string): Promise<{ questions: Question[] }> => {
    const response = await api.post(`/contests/${id}/generate-quiz-template`, { preview: true });
    return response.data;
  },

  saveQuizTemplate: async (id: string, questions: Question[]) => {
